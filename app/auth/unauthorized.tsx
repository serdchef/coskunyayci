...
export default function UnauthorizedPage() {
  ...
  const reason = searchParams?.get('reason') || 'insufficient-role';
  const requiredRole = searchParams?.get('requiredRole') || 'SUPER_ADMIN|ADMIN';
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-900 via-purple-900 to-slate-900 flex items-center justify-center px-4 py-12">
      ...
      <div className="absolute inset-0 opacity-5">
        <svg width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
          <pattern id="ottoman" x="0" y="0" width="80" height="80" patternUnits="userSpaceOnUse">
            <rect x="0" y="0" width="80" height="80" fill="none" stroke="gold" strokeWidth="0.5" />
            <circle cx="40" cy="40" r="20" fill="none" stroke="gold" strokeWidth="0.5" />
            <path d="M 40 20 L 50 30 L 40 40 L 30 30 Z" fill="none" stroke="gold" strokeWidth="0.5" />
          </pattern>
      ...
      <div className="relative max-w-md w-full">
        ...
        <div className="bg-white/10 backdrop-blur-xl rounded-2xl border border-white/20 shadow-2xl p-8">
          ...
          <p className="text-white/70 text-center text-sm mb-6">
            ...
            {reason === 'insufficient-role' && (
              <>
                Bu sayfaya erişim yetkiniz yoktur. Gerekli rol:{' '}
          ...
          {session?.user && (
            <div className="mb-6 p-4 bg-white/5 rounded-lg border border-white/10">
              <p className="text-xs text-white/60 mb-1">Giriş yapan kullanıcı:</p>
              <p className="text-white font-semibold text-sm">{session.user.email}</p>
              {session.user.role && (
                <p className="text-xs text-white/60 mt-2">
                  Rol: <span className="text-white/80">{session.user.role}</span>
                </p>
              )}
            </div>
          )}
          ...
          <div className="space-y-3">
            {session?.user ? (
              <>
                ...
                >
                  {isSigningOut ? 'Çıkış yapılıyor...' : 'Çıkış Yap ve Yeniden Giriş'}
                </button>
                <button
        ...
        {/* Decorative Elements */}
        <div className="absolute -top-4 -left-4 w-20 h-20 bg-purple-500/20 rounded-full blur-2xl" />
